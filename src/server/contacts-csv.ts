...
export type ImportReport = {
  ...
  skipped: string[];
  /** Header columns the import does not know and therefore ignores. */
 ...
 *
 * `dryRun` walks the file and matches every row without writing, so the
 * screen can show "12 atualizados, 3 ignorados" before the owner commits.
 */
export async function importContactsCsv(
  ...
  const known = new Set<string>(FIXED_COLUMNS);
  const header = records[0] ? Object.keys(records[0]) : [];
  const report: ImportReport = {
    total: records.length,
    updated: 0,
    skipped: [],
    unknownColumns: header.filter((h) => !known.has(h) && !h.startsWith(FIELD_PREFIX)),
  };
  ...
  for (const rec of records) {
    ...
    if (write && Object.keys(data).length)
    ...
    if (rec.tags !== undefined && rec.tags !== "") {
      ...
      for (const name of wanted)
        if (write && !current.has(name)) await addTagToContact(db, contact.id, name, "csv");
      for (const name of current)
        if (write && !wanted.has(name)) await removeTagFromContact(db, contact.id, name, "csv");
    }

    if (rec.subscribed?.trim()) {
      const on = TRUE_WORDS.has(rec.subscribed.trim().toLowerCase());
      if (write && on !== contact.subscribed) await setContactSubscribed(db, contact.id, on, "csv");
    }

    for (const [col, value] of Object.entries(rec)) {
      if (!write || !col.startsWith(FIELD_PREFIX) || value === "") continue;
